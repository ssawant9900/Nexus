...
function NexusMark() {
  return (
    <div className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-md border border-cyan-500/20 bg-cyan-500/10 text-cyan-500">
...
<aside className={`${sidebarOpen ? 'w-[17rem]' : 'w-0 overflow-hidden'} fixed inset-y-0 z-30 flex shrink-0 flex-col border-r border-slate-200 bg-white transition-[width] duration-300 dark:border-slate-800/60 dark:bg-[#0a0e17] lg:sticky lg:top-0 lg:h-screen lg:w-[17rem]`}>         <div className="flex flex-col border-b border-slate-200 p-5 dark:border-slate-800/60">
          <div className="flex items-center gap-3">
            <NexusMark/>
            <div>
              <p className="text-sm font-bold tracking-widest text-slate-900 dark:text-white">NEXUS</p>
              <p className="text-[10px] font-semibold tracking-widest text-cyan-600 dark:text-cyan-500">AI DELIVERY CONTROL</p>
            </div>
          </div>
            ...
            <div key={group.label}>
              ...
              <div className="space-y-1">
                {group.items.map((item) => (
                  <NavigationItem key={item.id} item={item} active={activeTab === item.id} onClick={() => setActiveTab(item.id)}/>
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="border-t border-slate-200 p-4 dark:border-slate-800/60">
          <NavigationItem item={{ id: 'settings', label: 'Project settings', icon: SettingsIcon }} active={activeTab === 'settings'} onClick={() => setActiveTab('settings')}/>
          <div className="mt-4 flex items-center gap-3 rounded-md px-2 py-2">
            <div className="grid h-8 w-8 place-items-center rounded-full bg-cyan-600 text-[11px] font-bold text-white shadow-sm">SK</div>
            <div className="flex-1">
              <p className="text-xs font-semibold text-slate-900 dark:text-slate-200">Shubham Kumar</p>
              <p className="text-[10px] text-slate-500">Project Administrator</p>
            </div>
      ...
      <main className="min-w-0 flex-1 flex flex-col">
        <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white/80 px-4 backdrop-blur-md dark:border-slate-800/60 dark:bg-[#030712]/80 md:px-8">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8">
          <div className="mx-auto max-w-[1400px]">
            ...
            {activeTab === 'pipelines' && <Pipelines onNavigate={setActiveTab}/>}
